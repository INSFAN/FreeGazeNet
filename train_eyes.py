...
from models.efficientnet_eyes import EfficientNet
from loss.loss import GazeLoss, L2Loss, L1Loss, L1andL2Loss
...
def train(device, train_loader, model, criterion, optimizer,
    ...
    for batch_idx, (patch, gaze_norm_g, head_norm, rot_vec_norm) in enumerate(train_loader):
        ...
        gaze_norm_gt = torch.clamp((100 * (gaze_norm_g + 0.75)).long(), -74, 75, out=None)

        gaze_p, gaze_y = model(patch)

        loss = criterion(gaze_p, gaze_norm_gt[:, 0]) + criterion(gaze_y, gaze_norm_gt[:, 1])
        ...
        gaze_pred = torch.cat([preds_p.view(patch.size(0), 1), preds_y.view(patch.size(0), 1)], 1)
        angle_error= mean_angle_error(gaze_pred.cpu().detach().numpy().astype(float)/100-0.75, 
...
def main(args):
 ...
 # optionally resume from a checkpoint
    ...
    # val_dataset = GazeCaptureDatasets(args.val_dataroot, train=False, transforms=transform)
    ...
    for epoch in range(args.start_epoch, args.end_epoch + 1):
        train_loss, train_error = train(args.device, train_dataloader, model,
                            criterion, optimizer, epoch, args)

        val_loss, val_error = validate(args.device, val_dataloader, model,
...
def parse_args():
    ...
    parser.add_argument('--device', default=torch.device("cuda" if torch.cuda.is_available() else "cpu"), type=str)  #TBD
    ...
    # parser.add_argument(
    #     '--dataroot',
    #     default='./data/MPIIFaceGazeEyes/norm_list.txt',
    #     type=str,
    #     metavar='PATH')
    # parser.add_argument(
    #     '--val_dataroot',
