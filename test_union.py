...
from models.efficientnet_eyes import EfficientNet, AuxiliaryNet
...
def validate(args, val_dataloader, model, auxiliarynet, criterion,
    ...
    with torch.no_grad():
        ...
        for i, (eyes, face, gaze_norm_g, head_norm, rot_vec_norm) in enumerate(val_dataloader):
            # measure data loading time
            data_time.update(time.time() - end)
            eyes = eyes.to(args.device)
            face = face.to(args.device)
            ...
            head_norm = 100 * head_norm
            gaze_norm_g = 100 * gaze_norm_g
            # loss = criterion(gaze_norm_g, head_norm, gaze_pred, head_pred)
            loss = criterion(gaze_norm_g, head_norm, gaze_pred[:,0:2], gaze_pred[:,2:4])
...
def main(args):
 ...
 # optionally resume from a checkpoint
    ...
    val_dataset = MPIIDatasets(args.val_dataroot, train=True, transforms=transform)
...
def parse_args():
    ...
    parser.add_argument(
        '--dataroot',
        default='./data/MPIIFaceGazeUnion/norm_list.txt',
        type=str,
        metavar='PATH')
    parser.add_argument(
        '--val_dataroot',
    ...
    # parser.add_argument(
    #     '--dataroot',
    #     default='./data/Gazecapture300Union/norm_list.txt',
    #     type=str,
    #     metavar='PATH')
    # parser.add_argument(
    #     '--val_dataroot',
